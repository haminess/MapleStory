...
{
	for (size_t i = 0; i < _Other.m_vecParentObjects.size(); ++i)
	{
		AddObject(_Other.m_vecParentObjects[i]->Clone(), false);
	}
}
...
{
	// 1. _Object가 소유한 자식들도 같이 해당 레이어에 이동하는 경우

	// 2. _Object가 소유한 자식들은 본래 레이어를 유지하는 경우
	// 2-1. _Object가 소유한 자식들 중에서 레이어가 소속되지 않은 경우, 부모 레이어로 설정한다.
