...
void CTileMap::SetTileColRow(UINT _Col, UINT _Row)
{
    ...
    for (size_t i = 0; i < m_vecTileInfo.size(); ++i)
    {
        ...
        info.SliceUV = Vector2(1.f / m_TileAtlasCol, 1.f / m_TileAtlasRow);
        //info.LeftTopUV = Vector2(info.SliceUV.x * (i % m_TileAtlasCol), info.SliceUV.y * (i / m_TileAtlasCol));
        info.LeftTopUV = Vector2(1.f / m_TileAtlasCol, 0.f);
...
{
    Transform()->SetRelativeScale(Vector3(m_Col * m_TileSizeX, m_Row * m_TileSizeY, 1.f));
}
...
void CTileMap::Render()
{
    ...
    // 재질 정보
    Vector2 vLeftTopUV = Vector2((float)m_TileAtlasSizeX * 6.f, 0.f) / Vector2(m_TileAtlas->GetWidth(), m_TileAtlas->GetHeight());
    Vector2 vTileSliceUV = Vector2((float)m_TileAtlasSizeX / (float)m_TileAtlas->GetWidth(), (float)m_TileAtlasSizeY / (float)m_TileAtlas->GetHeight());
